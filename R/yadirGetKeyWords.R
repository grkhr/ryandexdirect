yadirGetKeyWords <- function(CampaignIds = NULL,
  ...
  packageStartupMessage(paste0("awdwdwaffa ", nrow(result)), appendLF = T)
  packageStartupMessage(paste0("wafawffwa ", round(difftime(stop_time, start_time , units ="secs"),0), "awdwaddwa"), appendLF = T)
  #?????????? ?????????
  return(result)}
