yadirGetReportFun <- function(ReportType        = "CUSTOM_REPORT", 
  ...
  for(login in Login){
    ...
    packageStartupMessage(paste0("Loading data of ",login))
    ...
    if(answer$status_code == 201){
      packageStartupMessage("The report from ", DateFrom, " to ", DateTo, " has been successfully queued for offline generation.", appendLF = T)
    ...
    if(answer$status_code == 200){
      ...
      if (nrow(df_new) >= 1000000) {
        stop('You have reached the limits of Yandex.Direct API. Try to use "FetchBy" parameter with DateRangeType = "CUSTOM_DATE", "DateFrom" and "DateTo". If you are already using it, try to choose a smaller value.')
      }
  ...
  packageStartupMessage("Number of rows is ", nrow(result))
  packageStartupMessage("-----------------------------------------------------------")
  packageStartupMessage()
  # limits checker
  # return result
